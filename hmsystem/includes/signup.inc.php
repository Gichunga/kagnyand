...
if(isset($_POST['signup-submit']))
{
    ...
    //validation
    if((empty($firstname) || empty($lastname) || empty($phone) || empty($email) || empty($password) || empty($password_2) || empty($county)))
    {
    ...
    {
        header("location: ../signup.php?error=invaliddetails&first_name=".$firstname."&lastname=".$lastname."&email=".$email);
        exit();
    }
    else if(!preg_match("/^[a-zA-Z]*$/", $firstname))
    {
        header("location: ../signup.php?error=invaliddetails&lastname=".$lastname."&email=".$email."&phone_number=".$phone);
        exit();
    }
    else if(!preg_match("/^[a-zA-Z]*$/", $lastname))
    {
        header("location: ../signup.php?error=invaliddetails&first_name=".$firstname."&email=".$email."&phone_number=".$phone);
        exit();
    }
    ...
    else
    {
        ...
        else
        {
            ...
            {
                $sql = "INSERT INTO users(firstname, lastname, phone, email, county, password) VALUES(?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                if(!mysqli_stmt_prepare($stmt, $sql))
                {
                ...
                else
                {
                    ...
                    mysqli_stmt_bind_param($stmt, "ssssss", $firstname, $lastname, $phone, $email, $county, $hashPassword);
